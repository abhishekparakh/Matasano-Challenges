...
def decrypt(ciphertextInBytes):
    ...
    for KEYSIZE in range(2,40):
        distTemp = 0
        count = 0
        chunks = [ciphertextInBytes[i:i + KEYSIZE] for i in range(0, len(ciphertextInBytes), KEYSIZE)]
        #print(len(chunks))
    ...
    for i in range(keySize):
        chunk = ciphertextInBytes[i::keySize]   #returns every keySize_th item
        key += findKey(chunk)
