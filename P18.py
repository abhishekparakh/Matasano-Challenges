    ...
    def decrypt(self, dataInBytes):       #input as bytes
        ...
        for i in range(0, len(dataInBytes), 16):
            ...
            for a,b in zip(valueToXORBin, currentChunkBin):
                decrypted += str(int(a)^int(b))      #xor bytes and convert to chr

            decryptedBytes = BitArray(decrypted).bytes
            decryptedText += str(decryptedBytes, 'utf-8')

        return decryptedText       #returns a string always
